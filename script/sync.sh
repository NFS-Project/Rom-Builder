...
export command="$(head $HOME/script/build_rom.sh -n $(expr $(grep '# build rom' $HOME/script/build_rom.sh -n | cut -f1 -d:) - 1))"
export only_sync=$(grep 'repo sync' $HOME/script/build_rom.sh)

mkdir -p $HOME/ccache/$rom_name/$device
mkdir -p $HOME/$rom_name
cd $HOME/$rom_name
